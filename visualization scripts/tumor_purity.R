...
for ( name in twck_samples ) {
  ...
  if ( grepl("Re", name) == FALSE ) {
    ...
    new_name <- paste(str_sub(unlist(str_split(name, '-'))[2], 1, 9), str_sub(name, -1, -1), sep = '-')
...
# create plot with df.spread
ggplot(df.spread, aes(x=))
...
for ( tumor in htc_tumors ) {
  ...
  if ( nrow(count(unique(as.vector(tumor_df$Sample)))) > 1 ) {
  df.spread <- tumor_df %>% 
    spread(Sample, Value)
  df.spread[is.na(df.spread)] <- 0
